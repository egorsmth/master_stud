...
def experiment_1_basic_comparison():
    ...
    config = SVMConfig(C=C, learning_rate=lr, max_iter=max_iter, tol=1e-6, verbose=False)
    
    # Add all methods
    runner.add_method("1. Primal GD", 
                     PrimalSVM(config))
    
    runner.add_method("2. Dual Projection (Standard GD)",
                     ProjectionDualSVM(config, use_nesterov=False))
    
    runner.add_method("3. Dual Projection (Nesterov)",
                     ProjectionDualSVM(config, use_nesterov=True, momentum=0.9))
    
    runner.add_method("4. Dual Penalty (Standard GD)",
                     PenaltyDualSVM(config, use_nesterov=False, penalty_weight=10))
    
    runner.add_method("5. Dual Penalty (Nesterov)",
...
def experiment_3_learning_rate_study():
    ...
    # F1 vs learning rate
    axes[0].plot([r['lr'] for r in results['Nesterov']], 
                [r['f1'] for r in results['Nesterov']],
                'o-', label='Nesterov', linewidth=2, markersize=8)
    axes[0].plot([r['lr'] for r in results['Standard GD']], 
                [r['f1'] for r in results['Standard GD']],
                's-', label='Standard GD', linewidth=2, markersize=8)
    ...
    axes[1].plot([r['lr'] for r in results['Nesterov']], 
                [r['iterations'] for r in results['Nesterov']],
                'o-', label='Nesterov', linewidth=2, markersize=8)
    axes[1].plot([r['lr'] for r in results['Standard GD']], 
...
def experiment_4_momentum_study():
    ...
    for momentum in momentum_values:
        ...
        model = ProjectionDualSVM(config, use_nesterov=(momentum > 0), momentum=momentum)
        ...
        results.append({
            ...
            'final_grad_norm': result.gradient_norm_history[-1] if result.gradient_norm_history else None
        })
    ...
    # F1 vs momentum
    axes[0].plot([r['momentum'] for r in results], 
    ...
    axes[0].grid(True, alpha=0.3)
    axes[0].axvline(x=0.9, color='red', linestyle='--', alpha=0.5, label='Standard choice')
    axes[0].legend()
    
    # Iterations vs momentum
    axes[1].plot([r['momentum'] for r in results], 
    ...
    axes[2].plot([r['momentum'] for r in results], 
...
def experiment_5_projection_vs_penalty():
    ...
    penalty_weights = [10, 50, 100, 200, 500]
    
    # Add projection method (baseline)
    config = SVMConfig(C=0.1, learning_rate=1e-4, max_iter=2000, tol=1e-6)
    runner.add_method("Projection (Nesterov)",
                     ProjectionDualSVM(config, use_nesterov=True, momentum=0.9))
    
    # Add penalty methods with different weights
    for pw in penalty_weights:
        runner.add_method(f"Penalty (weight={pw})",
                         PenaltyDualSVM(config, use_nesterov=True, 
                                       momentum=0.9, penalty_weight=pw))
    ...
    for name, result in runner.results.items():
        if result.constraint_violation_history:
            axes[0].semilogy(result.constraint_violation_history, 
    ...
    for name, result in runner.results.items():
        methods.append(name)
        final_viols.append(result.constraint_violation_history[-1] 
                          if result.constraint_violation_history else 0)
        test_f1s.append(result.test_f1)
    
    axes[1].scatter(final_viols, test_f1s, s=100, alpha=0.7)
    for i, method in enumerate(methods):
        axes[1].annotate(method, (final_viols[i], test_f1s[i]), 
                        fontsize=8, ha='right')
