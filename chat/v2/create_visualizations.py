...
def create_penalty_weight_figure():
    ...
    # Panel 1: F1 Score
    ax = axes[0, 0]
    colors = ['green' if w <= 1 else 'yellow' if w <= 5 else 'orange' if w < 100 else 'red' 
              for w in weights]
    bars = ax.bar(range(len(weights)), f1_scores, color=colors, edgecolor='black', linewidth=2)
    ax.axhline(y=proj_f1, color='purple', linestyle='--', linewidth=2.5, label=f'Projection: {proj_f1:.4f}')
    ...
    ax.set_title('A. F1 Score: Very Light Penalty Wins!', fontsize=14, fontweight='bold')
    ...
    ax.semilogy(range(len(weights_stable)), kkt_stable, 'o-', 
                color='steelblue', linewidth=3, markersize=10, label='Penalty Method')
    ax.axhline(y=proj_kkt, color='purple', linestyle='--', linewidth=2.5,
              label=f'Projection: {proj_kkt:.1f}')
    ax.plot(0, kkt_stable[0], 'r*', markersize=20, markeredgecolor='black', 
            markeredgewidth=2, label=f'Best: {kkt_stable[0]:.2f}')
    
    ax.set_ylabel('KKT Violation (log scale)', fontsize=13, fontweight='bold')
    ax.set_xlabel('Penalty Weight', fontsize=13, fontweight='bold')
    ax.set_title('B. KKT Violation: 9× Better Than Projection!', fontsize=14, fontweight='bold')
    ...
    bars = ax.bar(range(len(weights_stable)), iterations_stable, 
                  color=colors[:-1], edgecolor='black', linewidth=2)
    ax.axhline(y=proj_iter, color='purple', linestyle='--', linewidth=2.5,
              label=f'Projection: {proj_iter}')
    
    for bar, it in zip(bars, iterations_stable):
        ax.text(bar.get_x() + bar.get_width()/2., bar.get_height() + 10,
               f'{it}', ha='center', va='bottom', fontsize=10, fontweight='bold')
    ...
    bars = ax.barh(range(len(methods)), f1_final, color=colors_final, 
                   edgecolor='black', linewidth=2)
    
    medals = ['🥇', '🥈', '🥉', '']
    for i, (bar, f1, medal) in enumerate(zip(bars, f1_final, medals)):
        ax.text(bar.get_width() + 0.001, bar.get_y() + bar.get_height()/2,
               f' {f1:.4f} {medal}', va='center', fontsize=12, fontweight='bold')
    ...
    fig.suptitle('Very Light Penalty Discovery: Key Findings', 
...
def create_bug_fix_comparison():
    ...
    ax.bar(x - width/2, primal_before, width, label='Primal', color='steelblue', 
           edgecolor='black', linewidth=2)
    ax.bar(x + width/2, dual_before, width, label='Dual', color='coral',
           edgecolor='black', linewidth=2)
    ...
    ax.bar(x - width/2, primal_after, width, label='Primal', color='steelblue',
           edgecolor='black', linewidth=2)
    ax.bar(x + width/2, dual_after, width, label='Dual', color='coral',
           edgecolor='black', linewidth=2)
    ...
    plt.suptitle('Bug Fix Validation: Accuracy Now Matches F1', 
