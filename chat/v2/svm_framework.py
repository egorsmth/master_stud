...
class BaseSVM(ABC):
    ...
    def evaluate(self, X: np.ndarray, y: np.ndarray) -> Tuple[float, float]:
        ...
        # Convert predictions from {-1, +1} to {0, 1} to match y
    ...
    def _check_convergence(self, gradient_norm: float, 
        ...
        if gradient_norm < self.config.tol:
            if self.config.verbose:
                print(f"Converged: gradient norm {gradient_norm:.2e} < {self.config.tol}")
            return True
        
        if iteration > 10 and obj_change < self.config.tol:
            if self.config.verbose:
                print(f"Converged: objective change {obj_change:.2e} < {self.config.tol}")
            return True
...
class PrimalSVM(BaseSVM):
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) -> OptimizationResult:
        ...
        for iteration in range(self.config.max_iter):
            ...
            # Check convergence
            obj_change = abs(obj_history[-1] - obj_history[-2]) if len(obj_history) > 1 else float('inf')
...
class DualSVM(BaseSVM):
    ...
    def compute_dual_objective(self, alpha: np.ndarray, 
                              K: np.ndarray, 
                              y: np.ndarray) -> float:
        """
    ...
    def compute_dual_gradient(self, alpha: np.ndarray,
                             K: np.ndarray,
    ...
    def reconstruct_primal_weights(self, alpha: np.ndarray,
                                   X: np.ndarray, 
                                   y: np.ndarray) -> np.ndarray:
        """Reconstruct w from dual solution: w = ∑αᵢyᵢxᵢ"""
        return np.sum(alpha[:, np.newaxis] * y[:, np.newaxis] * X, axis=0)
    
    def compute_kkt_violation(self, alpha: np.ndarray,
                             X: np.ndarray,
                             y: np.ndarray) -> float:
        """
...
class ProjectionDualSVM(DualSVM):
    ...
    def __init__(self, config: SVMConfig, use_nesterov: bool = False, 
                 momentum: float = 0.9):
...
class PenaltyDualSVM(DualSVM):
    ...
    def __init__(self, config: SVMConfig, use_nesterov: bool = False,
                 momentum: float = 0.9, penalty_weight: float = 100.0):
        super().__init__(config)
        self.use_nesterov = use_nesterov
        self.momentum = momentum
        self.penalty_weight = penalty_weight
    
    def compute_penalized_objective(self, alpha: np.ndarray,
                                    K: np.ndarray,
                                    y: np.ndarray) -> float:
    ...
    def compute_penalized_gradient(self, alpha: np.ndarray,
                                   K: np.ndarray,
                                   y: np.ndarray) -> np.ndarray:
